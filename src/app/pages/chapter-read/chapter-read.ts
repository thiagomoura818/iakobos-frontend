...
export class ChapterRead implements OnInit {
  ...
  ngOnInit(): void {
    const a = this.route.snapshot.paramMap.get('sigla');
    const t = this.route.snapshot.paramMap.get('traducao');
    const c = this.route.snapshot.paramMap.get('capitulo');
